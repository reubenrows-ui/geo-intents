...
entity_extractor_agent = LlmAgent(
    ...
    - What cafés are within 1 mile of Times Square?
        - Payload: anchor_place: Times Square, radius_value: 1, radius_unit: mile, entity: café
    - How many competitors are near my downtown Vancouver store?”
        - Payload: anchor_place: downtown Vancouver store, radius_value: null, radius_unit: null, entity: competitors
    - List cafe's  close to Union Station, Toronto.”
        - Payload: anchor_place: Union Station, Toronto, radius_value: null, radius_unit: null, entity: cafe's
...
entity_validator_agent = LlmAgent(
    name="AnchorPlaceValidator",
    input_schema=SlotCandidates,
    instruction=f"""You are an agent that validates if the extracted entity matches any known places in the datastore.
    Use the provided tool to search the datastore for places matching the extracted entity.
        If you are confident there is a match, return the latitude and longitude of the best matching place.
        If no match is found, return document_name as an empty response for latitude and longitude.
    Respond ONLY with a JSON object matching this exact schema:
    {json.dumps(PoiCoords.model_json_schema(), indent=2)}
    ...
    """,
    tools = [VertexAiSearchTool(data_store_id=DATASTORE_PATH,max_results=1)],
    output_key="entity_coords",

) # Custom non-LLM agent

nearby_intent_agent= SequentialAgent(
    name="nearby_intent_agent",
    sub_agents=[entity_extractor_agent, anchor_place_validator_agent, entity_validator_agent])